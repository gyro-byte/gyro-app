// Gyro Proxy — Service Worker
// Based on GhostLink proxy engine (Scramjet + BareMux)
    ...
    if (!currentOk) {
        ...
        if (best) {
            ...
            notifyClients({
                type: 'wispChanged',
                url: best.url,
                name: wispConfig.servers.find(s => s.url === best.url)?.name || 'Server',
